using BikeClub.Data;
using BikeClub.Features.Bike.Shared;
using BikeClub.SharedKernel.Results;
using FluentValidation;
using Microsoft.EntityFrameworkCore;
using BikeEntity = BikeClub.Domain.Entities.Bike;

namespace BikeClub.Features.Bike.UpdateBike;

internal sealed class UpdateBikeHandler
{
    private readonly DataContext _db;
    private readonly IValidator<UpdateBikeRequest> _validator;

    public UpdateBikeHandler(DataContext db, IValidator<UpdateBikeRequest> validator)
    {
        _db = db;
        _validator = validator;
    }

    public async Task<Result<BikeResponse>> Handle(int id, UpdateBikeRequest request, CancellationToken ct)
    {
        if (id != request.Id)
            return BikeErrors.IdMismatch;

        var validation = await _validator.ValidateAsync(request, ct);
        if (!validation.IsValid)
            return ValidationResult<BikeResponse>.WithErrors(
                validation.Errors
                    .Select(e => new Error(e.PropertyName, e.ErrorMessage, ErrorType.Validation))
                    .ToArray());

        var bike = new BikeEntity
        {
            Id = request.Id,
            Gears = request.Gears,
            FrameSize = request.FrameSize,
            RimSize = request.RimSize,
            Model = request.Model,
            Description = request.Description,
            Price = request.Price,
            Image = request.Image,
            GenderCode = request.GenderCode,
            CategoryId = request.CategoryId
        };

        _db.Entry(bike).State = EntityState.Modified;
        await _db.SaveChangesAsync(ct);

        return BikeResponse.From(bike);
    }
}
