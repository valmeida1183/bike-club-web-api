...
- **`result-pattern`** — per-operation `Result<T>` / `Result`.
- **`minimal-api`** — one endpoint per operation.
</skills>
...
Payload shapes must stay byte-identical to the current controllers. Double-check field names, nullable handling, and list ordering (current controllers do not `OrderBy`; neither should the new handlers).
...
- All happy-path responses identical to today.
...
- [ ] **Manual Verification**
  - [ ] Anon: `GET /v1/categories` → 200, cached on second call within 30s (same `Age` header or short round-trip).
  - [ ] Anon: `GET /v1/difficulties` and `GET /v1/genders` — same caching behavior.
  - [ ] Cyclist token: `GET /v1/roles` → 200. Anonymous: `GET /v1/roles` → 401.
  - [ ] Monitor: `POST /v1/categories` valid body → 200. Cyclist: same request → 403.
  - [ ] Monitor: `PUT /v1/difficulties/1` with body `{ "id": 2, ... }` → 400 `Difficulty.IdMismatch`.
  - [ ] Monitor: `PUT /v1/roles/Cyclist` with body `{ "name": "Monitor", ... }` → 400 `Role.NameMismatch` (case-insensitive comparison preserved).
  - [ ] `GET /v1/categories/9999` → 404 ProblemDetails.
